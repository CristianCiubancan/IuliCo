namespace IuliCo.Game.Cryptography
{
    ...
    public unsafe sealed class RivestCipher5
    {
        ...
        {
            _initializationVector = new byte[SEED_SIZE];
        ...
        public void GenerateKeys(byte[] initializationVector)
        {
            ...
            // Convert byte array to uint array before copying
            for (int i = 0; i < _keyBuffer.Length; i++)
            {
                _keyBuffer[i] = BitConverter.ToUInt32(initializationVector, i * sizeof(uint));
            }
        ...
        //         Core.Kernel.memcpy((byte*)keyBufferPtr, initializationVector, KEY_SIZE * sizeof(uint));
