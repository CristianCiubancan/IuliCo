...
namespace IuliCo.Account.Account.Client
{
    ...
    {
        public static byte[] Key = new byte[16] { 0x4A, 0x6F, 0x6E, 0x61, 0x74, 0x68, 0x61, 0x6E, 0x20, 0x4D, 0x61, 0x6E, 0x6E, 0x69, 0x6E, 0x67 };
        public static byte[] Salt = new byte[8] { 0x4A, 0x6F, 0x6E, 0x61, 0x74, 0x68, 0x61, 0x6E };
        private RivestCipher5 _rivest;
        public TransferCipher(string unique)
        {
            TigerHashAlgorithm tigerHash = new TigerHashAlgorithm();
            tigerHash.Hash(Encoding.UTF8.GetBytes(unique));
            tigerHash.Hash(Key);
        ...
        public TransferCipher(string key, string salt, string unique)
        {
            TigerHashAlgorithm tigerHash = new TigerHashAlgorithm();
            tigerHash.Hash(Encoding.UTF8.GetBytes(unique));
            tigerHash.Hash(Encoding.UTF8.GetBytes(key));
            PasswordDeriveBytes password = new PasswordDeriveBytes(tigerHash.Final(0x10), Encoding.UTF8.GetBytes(salt));
            _rivest = new RivestCipher5();
            _rivest.GenerateKeys(password.GetBytes(16));
        }
